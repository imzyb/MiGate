...
**Goal:** Build MiGate as an all-in-one Xray + VPNGate + OpenVPN + policy-routing smart egress gateway, so users can create inbound nodes, bind them to VPNGate exits, and generate share/subscription links without separately installing 轻量面板.
...
MiGate v0.1 is not a full 轻量面板 clone. It is a focused one-stop gateway:
...
5. Import/export compatible with 轻量面板-style node/user data.
6. Optional external 轻量面板 interop mode.
7. Automated TLS certificate management.
8. CDN/WebSocket templates.

## Implementation Notes

- Do not fork 轻量面板 for v0.1. Use it as UX inspiration only.
