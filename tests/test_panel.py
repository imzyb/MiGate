...
def test_panel_base_path_routes_login_home_logout_and_node_creation(tmp_path):
    ...
    created = client.post(
        "/api/nodes",
        json={"protocol": "vless", "host": "secret.example.com", "port": 443, "name": "Secret", "credential": "secret-uuid"},
    )
    assert created.status_code == 200
    assert "vless://secret-uuid@secret.example.com:443" in created.json()["share_link"]
...
def test_panel_node_toggle_disable_excludes_node_from_export_and_xray_preview(tmp_path):
    ...
    response = client.post(f"/api/nodes/{first.id}/disable")
...
def test_panel_node_delete_removes_node_from_web_api_and_export(tmp_path):
    ...
    response = client.post(f"/api/nodes/{doomed.id}/delete")
...
def test_panel_create_vless_node_returns_share_link_and_subscription(tmp_path):
    ...
    assert "vless://00000000-0000-4000-8000-000000000001@example.com:443" in payload["share_link"]
...
def test_panel_save_xray_config_writes_preview_to_config_path(tmp_path):
    ...
    response = client.post("/api/xray/config/save")
...
def test_panel_validate_xray_config_shows_result(tmp_path):
    ...
    assert "config ok" in payload.get("stdout", "")
...
def test_panel_save_systemd_units_writes_unit_files_only(tmp_path):
    ...
    response = client.post("/api/systemd/units/save")

    assert response.status_code == 200
    payload = response.json()
    assert payload["status"] == "saved"
    assert str(unit_dir / "migate-xray.service") in payload["units"]
    assert str(unit_dir / "migate-panel.service") in payload["units"]
...
def test_panel_xray_runtime_refresh_shows_not_installed_guidance(tmp_path):
    ...
    assert payload["bin_path"] == "/usr/local/bin/xray"
...
def test_panel_xray_install_plan_refresh_shows_preview(tmp_path):
    ...
    response = client.get("/api/xray/install-plan")

    assert response.status_code == 200
    payload = response.json()
    assert payload["version"] == "v1.8.24"
    assert "linux" in payload["system"]
    assert payload["steps"][0]["action"] == "verify_version"
...
def test_panel_xray_install_dry_run_shows_structured_preview_without_side_effects(tmp_path):
    ...
    response = client.get("/api/xray/install/dry-run")

    assert response.status_code == 200
    payload = response.json()
    assert payload["status"] == "dry_run"
    assert payload["message"] == "planned only; no commands executed"
    assert payload["steps"][0]["action"] == "download_archive"
    assert "curl -fsSL" in payload["steps"][0]["command_preview"]
    assert payload["performed_side_effects"] is False
...
def test_panel_xray_install_dry_run_rejection_is_rendered_safely(tmp_path):
    ...
    assert "refuses plans with side effects" in payload["message"]
    assert payload["performed_side_effects"] is False
...
def test_panel_dangerous_xray_html_posts_require_confirmation_when_enabled(tmp_path):
    ...
    assert apply_response.status_code == 403
    assert restart_response.status_code == 403
    assert apply_response.json()["status"] == "confirmation_required"
    assert apply_response.json()["required_confirm"] == "APPLY"
    assert restart_response.json()["status"] == "confirmation_required"
    assert restart_response.json()["required_confirm"] == "RESTART"
    assert not config_path.exists()
    assert calls == []
...
def test_panel_xray_apply_api_writes_validates_reloads_restarts_and_returns_json(tmp_path):
    ...
    repo.create_node(
        protocol="vless",
        name="Apply API Node",
        host="apply-api.example.com",
        port=443,
        credential="apply-api-uuid",
        share_link="vless://apply-api-uuid@apply-api.example.com:443#ApplyAPI",
        subscription="apply-api-subscription",
    )
    config_path = tmp_path / "etc" / "migate" / "xray" / "config.json"
    calls = []
    ...
    assert payload == {
        ...
        "restart": {"service": "migate-xray.service", "status": "success", "returncode": 0, "stdout": "restart ok", "stderr": ""},
        "services": {
            "migate-xray.service": {"status": "success", "returncode": 0, "stdout": "migate-xray.service active", "stderr": ""},
...
def test_panel_xray_apply_api_stops_when_validation_fails(tmp_path):
    repo = NodeRepository(tmp_path / "migate.db")
    repo.initialize()
    repo.create_node(
        protocol="vless",
        name="Invalid Apply API Node",
        host="invalid-apply-api.example.com",
        port=443,
        credential="invalid-apply-api-uuid",
        share_link="vless://invalid-apply-api-uuid@invalid-apply-api.example.com:443#InvalidApplyAPI",
        subscription="invalid-apply-api-subscription",
    )
    config_path = tmp_path / "etc" / "migate" / "xray" / "config.json"
    calls = []
    ...
    assert response.json() == {
        ...
        "target_path": str(config_path),
        "counts": {"total_nodes": 1, "enabled_nodes": 1},
        "validation": {"status": "invalid", "returncode": 1, "stdout": "", "stderr": "bad config"},
        "daemon_reload": None,
        "restart": None,
        "services": None,
        "performed_side_effects": True,
...
def test_panel_xray_restart_dry_run_api_previews_steps_without_side_effects(tmp_path):
    ...
    response = client.get("/api/xray/restart/dry-run")
    ...
    safe_previews = dashboard_response.json()["actions"]["safe_previews"]
    assert {"name": "xray_restart_dry_run", "method": "GET", "path": "/api/xray/restart/dry-run"} in safe_previews
    assert not config_path.exists()
    assert calls == []


def test_panel_xray_restart_api_validates_reloads_restarts_and_returns_json(tmp_path):
    repo = NodeRepository(tmp_path / "migate.db")
    config_path = tmp_path / "etc" / "migate" / "xray" / "config.json"
    calls = []
...
def test_panel_xray_restart_api_stops_when_validation_fails(tmp_path):
    repo = NodeRepository(tmp_path / "migate.db")
    config_path = tmp_path / "etc" / "migate" / "xray" / "config.json"
    calls = []
...
def test_panel_xray_restart_api_stops_when_daemon_reload_fails(tmp_path):
    repo = NodeRepository(tmp_path / "migate.db")
    config_path = tmp_path / "etc" / "migate" / "xray" / "config.json"
    calls = []
...
def test_panel_xray_restart_api_reports_restart_failure_with_refreshed_services(tmp_path):
    repo = NodeRepository(tmp_path / "migate.db")
    config_path = tmp_path / "etc" / "migate" / "xray" / "config.json"
    calls = []
...
def test_panel_service_status_refresh_shows_structured_results(tmp_path):
    ...
    assert payload["services"]["migate-xray.service"]["status"] == "systemctl_not_found"
...
def test_panel_egress_status_refresh_renders_latest_readonly_report(tmp_path):
    ...
    assert payload["checks"][0]["message"] == "tun-migate interface exists"
    assert payload["checks"][3]["message"] == "egress is allowed"
    assert payload["performed_side_effects"] is False
...
def test_panel_home_shows_egress_dry_run_controls_without_real_actions(tmp_path):
    ...
    assert response.status_code == 200
    decoded = unescape(response.text)
    assert "Egress" in decoded
    assert "Dry-run" in decoded
...
def test_panel_egress_up_dry_run_renders_planned_openvpn_and_routing_commands(tmp_path):
    ...
    response = client.get("/api/egress/up/dry-run")

    assert response.status_code == 200
    payload = response.json()
    assert payload["status"] == "dry_run"
    assert payload["message"] == "planned only; no egress up commands executed"
    assert "openvpn --config /var/lib/migate/runtime/active.ovpn" in payload["commands_executed"][0]
    assert payload["performed_side_effects"] is False
...
def test_panel_egress_down_dry_run_renders_planned_cleanup_and_stop_commands(tmp_path):
    ...
    response = client.get("/api/egress/down/dry-run")

    assert response.status_code == 200
    payload = response.json()
    assert payload["status"] == "dry_run"
    assert payload["message"] == "planned only; no egress down commands executed"
    assert "ip route del default dev tun-migate table 100" in payload["commands_executed"][0]
    assert payload["performed_side_effects"] is False
...
def test_panel_home_shows_validation_gated_xray_restart_action(tmp_path):
    ...
    # SSR xray page still exists but we check JSON API for restart info
    dashboard = client.get("/api/dashboard")
    assert dashboard.status_code == 200
    actions = dashboard.json()["actions"]
    assert any(a["name"] == "xray_restart" for a in actions["dangerous_actions"])
...
def test_panel_xray_restart_does_not_touch_systemd_when_validation_fails(tmp_path):
    ...
    assert payload["validation"]["stderr"] == "bad config"
    assert calls == []
...
def test_panel_xray_restart_skips_restart_when_daemon_reload_fails(tmp_path):
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["status"] == "daemon_reload_failed"
    assert payload["daemon_reload"]["stderr"] == "daemon reload failed"
    assert payload["restart"] is None
    assert calls == [f"validate:{config_path}", "daemon-reload"]


def test_panel_xray_restart_reports_restart_failure_after_reload_success(tmp_path):
    repo = NodeRepository(tmp_path / "migate.db")
    config_path = tmp_path / "config.json"
    calls = []
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["status"] == "restart_failed"
    assert payload["restart"]["stderr"] == "restart failed"
    assert calls == [
        f"validate:{config_path}",
        "daemon-reload",
        "restart:migate-xray.service",
...
def test_panel_xray_restart_runs_daemon_reload_then_restart_after_valid_config(tmp_path):
    ...
    assert payload["validation"]["stdout"] == "config ok"
    assert payload["daemon_reload"]["stdout"] == "daemon ok"
    assert payload["restart"]["stdout"] == "restart ok"
    assert calls == [
        f"validate:{config_path}",
        "daemon-reload",
        "restart:migate-xray.service",
...
def test_panel_apply_current_nodes_saves_config_but_skips_systemd_when_validation_fails(tmp_path):
    ...
    payload = response.json()
    assert payload["status"] == "validation_failed"
    assert payload["validation"]["stderr"] == "invalid generated config"
    assert config_path.exists()
    assert calls == [f"validate:{config_path}"]
...
def test_panel_apply_current_nodes_reports_restart_failure_after_reload_success(tmp_path):
    repo = NodeRepository(tmp_path / "migate.db")
    repo.initialize()
    repo.create_node(
        protocol="vless",
        name="Restart Failure Node",
        host="restart-failure.example.com",
        port=443,
        credential="restart-failure-uuid",
        share_link="vless://restart-failure-uuid@restart-failure.example.com:443#RestartFailure",
        subscription="restart-failure-subscription",
    )
    config_path = tmp_path / "etc" / "migate" / "xray" / "config.json"
    calls = []
...
def test_panel_proxy_service_preview_api_returns_readonly_continuous_unit(tmp_path):
    ...
    assert "ExecStart=/usr/local/bin/migate proxy run --backend xray-tun --max-clients 0" in payload["content"]
...
def test_panel_xray_x25519_returns_html_with_keys(tmp_path):
    ...
    assert resp.status_code == 200
    data = resp.json()
    assert data["private_key"] == "testprivatekey"
    assert data["public_key"] == "testpublickey"
