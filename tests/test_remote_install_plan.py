...
def test_build_remote_install_dry_run_plan_lists_remote_command_previews_without_side_effects():
    ...
    assert plan == RemoteInstallPlan(
        ...
        steps=[
            ...
            RemoteInstallStep("install_python_package", "install MiGate package on remote host", "ssh -p 22 root@166.88.232.2 -- cd /tmp/migate-install && python3 -m pip install .", performs_side_effects=True),
