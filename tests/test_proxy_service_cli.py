...
def test_preview_proxy_service_unit_renders_safe_systemd_unit_without_side_effects():
    ...
    assert "ExecStart=/usr/local/bin/migate proxy run --backend xray-tun --max-clients 0" in unit
...
def test_save_proxy_service_unit_writes_unit_when_double_gate_passes(tmp_path):
    ...
    assert "ExecStart=/usr/local/bin/migate proxy run --backend xray-tun --max-clients 0" in content
...
def test_save_proxy_service_unit_allows_custom_migate_binary(tmp_path):
    ...
    assert result.status == "saved"
    assert "ExecStart=/opt/migate/bin/migate proxy run --backend xray-tun --max-clients 0" in target.read_text(encoding="utf-8")
