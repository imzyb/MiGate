...
def test_run_remote_install_plan_stops_on_first_failed_step():
    ...
    def runner(command: str) -> RemoteInstallCommandResult:
        calls.append(command)
        if "python3 -m pip install" in command:
    ...
    assert result.steps[-1] == RemoteInstallStepResult(
        ...
        description="install MiGate package on remote host",
        status="failed",
        command="ssh -p 22 root@166.88.232.2 -- cd /tmp/migate-install && python3 -m pip install .",
        returncode=1,
        stdout="",
        stderr="pip failed",
    )
